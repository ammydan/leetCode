...
//        2、参考思路：马拉车算法
        ...
        for(int i = 1;i<len-1;i++){
            opt[i] = i<mx?Math.min(opt[2*id-i],mx-i):1;
            while ((i - opt[i] >= 0) && ((i + opt[i]) < len - 1) && (str.charAt(i + opt[i]) == str.charAt(i - opt[i]))) {
                opt[i]++;
            }
            if(mx<i+opt[i]){
                mx = i+opt[i];
        ...
        return str.substring((center-ans)/2,(center-ans)/2-1+ans);
    }
...
//
    ...
    public static void main(String[] args) {
        ...
        System.out.println(test.longestPalindrome("abababba"));
//        "abacab"
//
    }
}
